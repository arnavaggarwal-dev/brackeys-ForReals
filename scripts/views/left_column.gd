...
static func _topic_board() -> Control:
	...
	for id: String in Data.TOPIC_ORDER:
		...
		row.add_child(Meter.make(clampf(heat, 0.0, 1.0), Style.topic_color(id), 0, 14))
...
static func _objective() -> Control:
	...
	col.add_child(rank)
	col.add_child(Meter.make(Game.objective_progress(), Style.BAR_FILL, 0, 18).follow(
