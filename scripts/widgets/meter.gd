...
static func make(v: float = 0.0, color: Color = Color("000080"), w: float = 0.0, h: float = 18.0) -> Meter:
	var m := Meter.new()
	m.value = clampf(v, 0.0, 1.0)
	m.ink = color
	m.custom_minimum_size = Vector2(w, h)
	if w <= 0.0:
