...
# Only the mouse is answered, emulated or real. Raw touch is left alone so it
# reaches the ScrollContainer above and the list still scrolls.
...
func _gui_input(event: InputEvent) -> void:
	...
	# A finger on a row may only be starting a scroll, so the press is dropped the
	# moment it travels.
