# Finger scrolling for a ScrollContainer. Every row is a Tappable that swallows
# the press before the container sees it, so the drag is tracked in _input and
# turned into a scroll offset directly. While one is in progress `scrolling` is
# true and Tappable throws the release away, so dragging never buys anything.
...
func _input(event: InputEvent) -> void:
	...
	# A real mouse is left alone, so click and drag on the desktop stays a click.
